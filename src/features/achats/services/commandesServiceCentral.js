...
export class CommandesService {
  ...
  static async getCommandesByFournisseur(fournisseurId) {
    const commandes = await centralDataService.getCommandes()
    return commandes.filter(commande => commande.fournisseurId === fournisseurId)
  }
  ...
  static async getCommandesByDateRange(startDate, endDate) {
    ...
    return commandes.filter(commande => {
      ...
      return commandeDate >= new Date(startDate) && commandeDate <= new Date(endDate)
    })
  }
  ...
  static async getCommandesWithFilters(filters) {
    ...
    if (filters.dateFrom) {
      commandes = commandes.filter(c => new Date(c.date) >= new Date(filters.dateFrom))
    }

    if (filters.dateTo) {
      commandes = commandes.filter(c => new Date(c.date) <= new Date(filters.dateTo))
    }

    if (filters.montantMin) {
      commandes = commandes.filter(c => c.montantTotal >= parseFloat(filters.montantMin))
    }

    if (filters.montantMax) {
      commandes = commandes.filter(c => c.montantTotal <= parseFloat(filters.montantMax))
    }
  ...
  static async getTopFournisseurs(limit = 5) {
    ...
    commandes.forEach(commande => {
      ...
      fournisseurStats[commande.fournisseur].total += commande.montantTotal || 0
