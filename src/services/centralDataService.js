...
class CentralDataService {
  ...
  async updateFournisseur(id, updatedData) {
    ...
    this.data.fournisseurs[index] = { ...this.data.fournisseurs[index], ...updatedData }
  ...
  async getCommandes() {
    ...
    await this.delay()
    return this.data.commandes_fournisseurs.find(commande => commande.id === id)
  }  async addCommande(newCommande) {
    await this.delay(500)
    const newId = Math.max(0, ...this.data.commandes_fournisseurs.map(c => c.id)) + 1
    const commandeToAdd = {
      ...newCommande,
      id: newId,
    ...
    await this.delay(300)
    const index = this.data.commandes_fournisseurs.findIndex(c => c.id === id)
  ...
  async refreshStats() {
    ...
    const stockStats = {
      ...
      lowStock: this.data.stock.filter(item => item.quantite <= item.seuilMinimum && item.quantite > 0).length,
      totalValue: this.data.stock.reduce((sum, item) => sum + (item.quantite * item.prixAchat), 0)
    }

    const fournisseursStats = {
      total: this.data.fournisseurs.length,
      actifs: this.data.fournisseurs.filter(f => f.statut === 'actif').length,      inactifs: this.data.fournisseurs.filter(f => f.statut === 'inactif').length,
      suspendus: this.data.fournisseurs.filter(f => f.statut === 'suspendu').length,
      totalChiffreAffaires: this.data.fournisseurs.reduce((sum, f) => sum + f.chiffreAffaires, 0)
    }

    const commandesStats = {
      total: this.data.commandes_fournisseurs.length,
      enAttente: this.data.commandes_fournisseurs.filter(c => c.statut === 'en_attente').length,
      livrees: this.data.commandes_fournisseurs.filter(c => c.statut === 'livre').length,
      annulees: this.data.commandes_fournisseurs.filter(c => c.statut === 'annule').length,
      totalValue: this.data.commandes_fournisseurs.reduce((sum, c) => sum + c.montantTTC, 0)
    }
