...
export function CommandeDetailModal({ commande, onClose, onEdit, onDownload }) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {      case STATUT_COMMANDE.PAYE:
        return (
          <Badge
            variant="secondary"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-xl font-semibold text-[var(--color-foreground)] mb-2">
            {commande.reference || `CMD-${commande.id.toString().padStart(4, "0")}`}
          </h3>
          <div className="flex items-center gap-4 text-sm text-[var(--color-foreground-muted)]">
            ...
            {commande.dateLivraison && (
              <div className="flex items-center gap-1">
                ...
                <span>Livraison prévue le {formatDate(commande.dateLivraison)}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">        {/* Informations Fournisseur */}
        ...
        <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200 shadow-sm">
          ...
          <CardContent className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">{formatCurrency(commande.montantHT || 0)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--color-foreground-muted)]">TVA</span>
              <span className="font-medium">{formatCurrency((commande.montantTTC || commande.montantTotal || 0) - (commande.montantHT || 0))}</span>
            </div>
            <div className="border-t border-[var(--color-border)] pt-2">
              <div className="flex justify-between items-center">
                <span className="text-base font-medium">Total TTC</span>
                <span className="text-lg font-bold text-[var(--color-blue)]">
                  {formatCurrency(commande.montantTTC || commande.montantTotal || 0)}
                </span>
              </div>
            </div>
                ...
                >
                  <div className="flex-1">
                    <h4 className="font-medium text-[var(--color-foreground)]">
                      {article.nom || article.designation || `Article ${index + 1}`}
                    </h4>
                    {article.description && (
